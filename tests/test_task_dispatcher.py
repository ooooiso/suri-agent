...
class TestMatchDepartment:
    @pytest.mark.asyncio
    async def test_match_by_keyword_engineering(self, task_service):
        """关键词匹配：开发相关需求应匹配 engineering 部门"""
        departments = [
            {"id": "engineering", "lead_role": "suri-dev"},
            {"id": "design", "lead_role": "suri-design"},
        ]
    ...
    @pytest.mark.asyncio
    async def test_match_by_keyword_design(self, task_service):
    ...
    @pytest.mark.asyncio
    async def test_match_by_keyword_hr(self, task_service):
        """关键词匹配：角色管理需求应匹配 hr 部门"""
        departments = [
            {"id": "hr", "lead_role": "suri-hr"},
            {"id": "engineering", "lead_role": "suri-dev"},
        ]
    ...
    @pytest.mark.asyncio
    async def test_match_fallback_to_central(self, task_service):
        """无关键词匹配时应回退到 central（而非第一个部门）"""
        departments = [
            {"id": "central", "lead_role": "suri"},
            {"id": "engineering", "lead_role": "suri-dev"},
        ]
...
class TestDispatch:
    ...
    async def test_dispatch_success(self, task_service, mock_services):
        ...
        config.get_function_index.return_value = MagicMock(
            meta={"departments": [{"id": "engineering", "lead_role": "suri-dev"}]}
        )
