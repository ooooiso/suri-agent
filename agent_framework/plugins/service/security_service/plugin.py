...
class SecurityServicePlugin(PluginInterface):
    ...
    async def _on_tool_call(self, event: Event) -> None:
        ...
        MCP Framework 是唯一的工具注册中心。
        安全检查通过后放行（重新发布给 mcp_framework 处理），
        检查不通过则发布 error.tool 事件并阻止调用。
        """
        tool_name = event.payload.get("tool_name", "")
        params = event.payload.get("params", {})
        
        # MCP Framework 文件工具（需要路径检查）
        FILE_READ_TOOLS = {"file_read", "file_list", "file_search", "file_stat"}
        FILE_WRITE_TOOLS = {"file_write", "file_append", "file_create"}
        
        path = params.get("path", "")
        
        # 只读操作检查
        if tool_name in FILE_READ_TOOLS and path:
        ...
        # 写操作检查
        if tool_name in FILE_WRITE_TOOLS and path:
        ...
        # 安全检查通过，重新发布给 mcp_framework
        await self._event_bus.publish(Event(
            event_type="tool.call",
            source=event.source,
            target="mcp_framework",
