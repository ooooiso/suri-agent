...
from agent_framework.shared.interfaces.plugin import PluginInterface, RuleProvider, TaskTemplate, TemplateStep
...
class CodeToolPlugin(PluginInterface, RuleProvider):
    ...
    async def init(self, event_bus, config: Dict[str, Any]) -> None:
        ...
        # 项目根目录 = suri-agent/ (向上4层: plugin.py → code_tool → plugins → agent_framework → project_root)
