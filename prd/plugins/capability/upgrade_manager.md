...
#### `learning.report_generated`
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `report_id` | string | 是 | 报告 ID |
| `module_id` | string | 是 | 模块 ID |
| `module_type` | string | 是 | plugin / tool / role / framework |
| `findings` | array | 是 | Finding 列表 |
| `priority` | string | 是 | CRITICAL / HIGH / MEDIUM / LOW |
| `summary` | string | 是 | 报告摘要 |

#### `upgrade.status_change_requested`
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `report_id` | string | 是 | 报告 ID |
| `new_status` | string | 是 | 新状态 |
| `reason` | string | 否 | 变更原因 |
| `changed_by` | string | 是 | 变更者 |

#### `upgrade.check_requested`
触发闭环检查，无特定 payload。

### 发布事件

#### `upgrade.report_saved`
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `report_id` | string | 是 | 报告 ID |
| `module_id` | string | 是 | 模块 ID |
...
#### `upgrade.reports_pending`
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `module_id` | string | 是 | 模块 ID |
| `count` | integer | 是 | 待处理报告数 |
| `report_ids` | array | 是 | 报告 ID 列表 |

### 4. 闭环检查（ClosedLoopCheck）

由 cron_service 定时触发，或系统级角色（如 suri）在系统维护时调用：
