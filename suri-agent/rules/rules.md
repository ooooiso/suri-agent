...
- **"核心记忆同步规则"** 一切给予角色。每次开发完成后（新增/修改/删除模块），必须调用 DocSyncService 检测代码变更，调用大模型生成文档更新摘要，经 document-review 审核后向用户汇报，用户确认"是"后方可写入 `suri-agent/memory/ai-dev-memory/`。AI 开发记忆库包含 architecture.md（架构决策）、development-log.md（开发日志）、module-index.md（模块索引），是 AI 开发上下文的唯一权威源。

- **"文档同步自动化规则"** 一切给予角色。DocSyncRule 引擎持续监控 suri-agent/、group/、wiki/ 下的代码文件变更，自动检测"代码已更新但同名 .md 未更新"的违规项。检测流程：
  1. DocWatcher 后台监控文件系统，捕获变更事件
  2. DocSyncRule.scan() 对比代码 mtime 与文档 mtime，标记缺失/过时项
  3. 生成同步计划，调用大模型生成更新建议
