...
| 角色 ID | 类型 | 核心能力 | 调度匹配关键词 |
|---------|------|---------|---------------|
| **suri** | 调度总监 | 需求解析、任务分发、跨部门协调、异常处理、汇总交付 | 任何用户需求的第一入口 |
| **suri-hr** | 角色管理 | 角色创建、能力分析、流程模板、组织架构维护 | 新建/修改/注销角色 |
| **suri-dev** | 程序维护 | Bug修复、代码升级、性能优化、框架维护 | 平台技术问题、升级需求 |

### 3.3 角色能力边界

**suri**
- can：接收并解析用户需求、匹配责任部门并下发任务、跟踪任务全生命周期、协调跨部门协作、处理异常回流与用户决策、汇总交付物并呈现给用户
